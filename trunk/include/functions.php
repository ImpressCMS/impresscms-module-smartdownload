...
function wfdownloads_icms_cp_header()
{
	icms_cp_header();
...
function wfd_serverstats()
{
	...
	echo "
		...
		<div>" . _AM_WFD_DOWN_METAVERSION . $meta . "</div>\n
		<div style='padding: 8px;'>\n
...
function wfd_displayicons($time, $status = 0, $counter = 0)
{
    ...
    if ($xoopsModuleConfig['displayicons'] != 3)
    {
        if ($newdate < $time)
        {
            if (intval($status) > 1)
            {
                if ($xoopsModuleConfig['displayicons'] == 1)
                    $new = "&nbsp;<img src=\"" . XOOPS_URL . "/modules/wfdownloads/images/icon/update.gif\" alt='Updated' align ='top'/>";
            ...
            {
                if ($xoopsModuleConfig['displayicons'] == 1)
                    $new = "&nbsp;<img src=\"" . XOOPS_URL . "/modules/wfdownloads/images/icon/newred.gif\" alt='New' align ='top'/>";
                if ($xoopsModuleConfig['displayicons'] == 2)
                    $new = "<i>New!</i>";
            }
        }
        if ($popdate < $time)
        {
            ...
            {
                if ($xoopsModuleConfig['displayicons'] == 1)
                    $pop = "&nbsp;<img src =\"" . XOOPS_URL . "/modules/wfdownloads/images/icon/pop.gif\" alt='Popular' align ='top'/>";
                if ($xoopsModuleConfig['displayicons'] == 2)
                    $pop = "<i>Popular</i>";
            }
        }
...
function wfd_PrettySize($size)
{
		...
		if($size < $kb) {
			return $size. _MD_WFD_BYTES;
		}
		else if($size < $mb) {
			return round($size/$kb,2). _MD_WFD_KILOBYTES;
		}
		else if($size < $gb) {
			return round($size/$mb,2). _MD_WFD_MEGABYTES;
		}
		else if($size < $tb) {
			return round($size/$gb,2). _MD_WFD_GIGABYTES;
		}
		else {
			return round($size/$tb,2). _MD_WFD_TERRABYTES;
		}
}
...
function wfd_updaterating($sel_id)
{
    ...
    $ratings = $rating_handler->getObjects(new icms_db_criteria_Item("lid", intval($sel_id)));
...
function wfd_totalcategory()
{
    ...
    //$gperm_handler = icms::handler('icms_member_groupperm');;
    $gperm_handler = $gperm_handler = icms::handler('icms_member_groupperm');
...
function wfd_getTotalItems($sel_id = 0, $ids = array())
{
    ...
    if ($sel_id > 0) {
    	$criteria->add(new icms_db_criteria_Item("cid", $sel_id));
    }
    if (count($ids) > 0) {
    	$criteria->add(new icms_db_criteria_Item("cid", "(".implode(',', $ids).")", "IN"));
    }
...
function down_createthumb($img_name, $img_path, $img_savepath, $width = 100, $height = 100, $quality = 100, $update = 0, $aspect = 1)
{
    ...
    if (!empty($img))
    {
         ...
         */
        ImageCopyResampled($tmp_img, $img, 0, 0, 0, 0, $width, $height, $orig_width, $orig_height);
        imagedestroy($img);
        flush();
        $img = $tmp_img;
    }
...
function wfd_downlistpagenav($pubrowamount, $start, $art = "art")
{
    ...
    $pagenav = new icms_view_PageNav($pubrowamount, $xoopsModuleConfig['admin_perpage'], $start, 'st' . $art);
