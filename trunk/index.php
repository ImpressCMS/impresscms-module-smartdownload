...
$icmsTpl->assign('wfdownloads_url', WFDOWNLOADS_URL);

$category_handler = icms_getModuleHandler('category');
$cat_criteria = new icms_db_criteria_Compo();
...
$icmsTpl->assign('catarray', $catarray);
/**
...
if($xoopsModuleConfig['subcats'] != 1) {
		...
		$icmsTpl->append('categories', array('image' => $imgurl,
...
$icmsTpl->assign('lang_thereare', sprintf($lang_ThereAre, $count, array_sum($listings['count'])));
$icmsTpl->assign('module_home', wfdownloads_module_home(false));

if($xoopsModuleConfig['enablerss'] == 1)
{
	$rsslink=sprintf("<a href='%s' title='%s'><img src='%s' alt='%s' title='%s' /></a>",WFDOWNLOADS_URL."rss.php", _MD_WFD_LEGENDTEXTRSS, XOOPS_URL."/".$xoopsModuleConfig['mainimagedir']."/icon/rss.gif",_MD_WFD_LEGENDTEXTRSS, _MD_WFD_LEGENDTEXTRSS);
	$icmsTpl->assign('full_rssfeed_link', $rsslink);
}
