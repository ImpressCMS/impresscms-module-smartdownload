...
$module_id = (int) $icmsModule->getVar('mid');
...
foreach (array_keys($top_cats) as $i) {
    ...
    if (in_array($cid, $allowed_cats)) {
        ...
        $criteria = new icms_db_criteria_Compo(new icms_db_criteria_Item("cid", "(".implode(',', $cids).")", "IN"));
        ...
        if ($filecount > 0) {
            ...
            foreach (array_keys($downloads) as $k) {
                ...
                $rankings[$e]['file'][] = array('id' => (int) $downloads[$k]->getVar('lid'),
