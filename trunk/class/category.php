<?php
// $Id: category.php,v 1.11 2007/09/30 13:47:53 m0nty_ Exp $
// ------------------------------------------------------------------------ //
...
class WfdownloadsCategory extends XoopsObject {
    ...
    function getForm($heading) {
        ...
        $sform = new icms_form_Theme($heading, "op", $_SERVER['REQUEST_URI']);
	...
	$mid = intval($xoopsModule->getVar('mid'));
        ...
        $groups = $groups;
        $sform -> addElement(new icms_form_elements_select_Group(_AM_WFD_FCATEGORY_GROUPPROMPT, "groups", true, $groups, 5, true));
        ...
        if ($totalcats > 0)
        {
            ...
            $sform->addElement(new icms_form_elements_Label(_AM_WFD_FCATEGORY_SUBCATEGORY, $mytree->makeSelBox('pid', 'title', "-", $this->getVar('pid', 'e'),true)));
    	}
        $sform -> addElement(new icms_form_elements_Text(_AM_WFD_FCATEGORY_TITLE, 'title', 50, 255, $this->getVar('title', 'e')), true);
        $sform -> addElement(new icms_form_elements_Text(_AM_WFD_FCATEGORY_WEIGHT, 'weight', 10, 80, $this->getVar('weight')), false);

        $graph_array = WfsLists :: getListTypeAsArray(XOOPS_ROOT_PATH . "/" . $xoopsModuleConfig['catimage'], $type = "images");
        $indeximage_select = new icms_form_elements_Select('', 'imgurl', $this->getVar('imgurl'));
        $indeximage_select -> addOptionArray($graph_array);
        $indeximage_select -> setExtra("onchange='showImgSelected(\"image\", \"imgurl\", \"" . $xoopsModuleConfig['catimage'] . "\", \"\", \"" . XOOPS_URL . "\")'");
        $indeximage_tray = new icms_form_elements_Tray(_AM_WFD_FCATEGORY_CIMAGE, '&nbsp;');
        $indeximage_tray -> addElement($indeximage_select);
        if ($this->getVar('imgurl') != "")
        {
            $indeximage_tray -> addElement(new icms_form_elements_Label('', "<br /><br /><img src='" . XOOPS_URL . "/" . $xoopsModuleConfig['catimage'] . "/" . $this->getVar('imgurl') . "' name='image' id='image' alt='' title='image' />"));
        }
        else
        {
        ...
        $sform -> addElement(new icms_form_elements_Dhtmltextarea(_AM_WFD_FCATEGORY_DESCRIPTION, 'description', $this->getVar('description', 'e'), 15, 60), true);
        $sform -> addElement(new icms_form_elements_Textarea(_AM_WFD_FCATEGORY_SUMMARY, 'summary', $this->getVar('summary'), 10, 60));

        $options_tray = new icms_form_elements_Tray(_AM_WFD_TEXTOPTIONS, '<br />');

        $html_checkbox = new icms_form_elements_Checkbox('', 'dohtml', intval($this->getVar('dohtml')));
        $html_checkbox -> addOption(1, _AM_WFD_ALLOWHTML);
        $options_tray -> addElement($html_checkbox);

        $smiley_checkbox = new icms_form_elements_Checkbox('', 'dosmiley', intval($this->getVar('dosmiley')));
        $smiley_checkbox -> addOption(1, _AM_WFD_ALLOWSMILEY);
        $options_tray -> addElement($smiley_checkbox);

        $xcodes_checkbox = new icms_form_elements_Checkbox('', 'doxcode', intval($this->getVar('doxcode')));
        $xcodes_checkbox -> addOption(1, _AM_WFD_ALLOWXCODE);
        $options_tray -> addElement($xcodes_checkbox);

        $noimages_checkbox = new icms_form_elements_Checkbox('', 'doimage', intval($this->getVar('doimage')));
        $noimages_checkbox -> addOption(1, _AM_WFD_ALLOWIMAGES);
        $options_tray -> addElement($noimages_checkbox);

        $breaks_checkbox = new icms_form_elements_Checkbox('', 'dobr', intval($this->getVar('dobr')));
		...
		if(file_exists(XOOPS_ROOT_PATH . "/modules/formulize/include/functions.php")) {
                ...
                $formulize_forms = new icms_form_elements_Select(_AM_WFD_FFS_CUSTOM_FORM, "formulize_fid", $this->getVar('formulize_fid') );
        ...
        else
        {
            ...
            $butt_create = new icms_form_elements_Button('', '', _AM_WFD_BMODIFY, 'submit');
            $butt_create -> setExtra('onclick="this.form.elements.op.value=\'addCat\'"');
            $button_tray -> addElement($butt_create);
...
class WfdownloadsCategoryHandler extends XoopsPersistableObjectHandler {
    ...
    function getNicePath($cid, $root_filename = "index.php", $item_filename = "viewcat.php?op=") {
        ...
        $mytree = new icms_view_Tree($this->table, $this->keyName, "pid");
...
//      $pathstring = substr($pathstring, 0, strlen($pathstring) - 13); // not needed now with fixed icms core! but required for XOOPS
    ...
    function getUserCategories($id_as_key = false, $as_object = true) {
        ...
        return $this->getObjects(new icms_db_criteria_Item('cid', "(".implode(',', $categoryids).")", "IN"), $id_as_key, $as_object);
    }

    function getChildCats($category) {
        $allcats = $this->getObjects();
        include_once(XOOPS_ROOT_PATH."/class/tree.php");
        $tree = new icms_ipf_Tree($allcats, $this->keyName, "pid");
	...
	function getAllSubcatsTopParentCid() {
		...
		include_once(XOOPS_ROOT_PATH."/class/tree.php");
	    $tree = new icms_ipf_Tree($this->allCategories, $this->keyName, "pid");
