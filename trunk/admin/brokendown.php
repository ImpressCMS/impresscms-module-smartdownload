...
switch ($op)
{
    ...
    if (isset($_GET['ack']))
    {
        ...
        $report_handler->updateAll("acknowledged", $acknowledged, new icms_db_criteria_Item("lid", $lid), true);
    ...
    if (isset($_GET['con']))
    {
        ...
        $report_handler->updateAll("confirmed", $confirmed, new icms_db_criteria_Item("lid", $lid), true);
    ...
    $criteria = new icms_db_criteria_Item('lid', intval($lid));
    $report_handler = icms_getModuleHandler('report');
    ...
    else
    {
        ...
        $download_handler = icms_getModuleHandler('download');
        $downloads = $download_handler->getObjects(new icms_db_criteria_Item('lid', "(".implode(',', array_unique($lids)).")", "IN"), true);
        ...
        $users = $member_handler->getUsers(new icms_db_criteria_Item("uid", "(".implode(',', array_unique($uids)).")", "IN"), true);
