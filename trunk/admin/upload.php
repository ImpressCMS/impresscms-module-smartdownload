...
<div><b>" . _AM_WFD_DOWN_FUPLOADURL . "</b> " . XOOPS_URL . "/" . $dirarray[$rootpath] . "</div><br />\n";
        ...
        $iform = new icms_form_Theme(_AM_WFD_DOWN_FUPLOADIMAGETO . $pathlist, "op", xoops_getenv('PHP_SELF'));
        $iform->setExtra('enctype="multipart/form-data"');

        ob_start();
        $iform->addElement(new icms_form_elements_Hidden('dir', $rootpath));
        wfd_getDirSelectOption($namelist, $dirarray, $namearray);
        $iform->addElement(new icms_form_elements_Label(_AM_WFD_DOWN_FOLDERSELECTION, ob_get_contents()));
        ob_end_clean();

        if ($rootpath > 0)
        {
            $graph_array = WfsLists::getListTypeAsArray(XOOPS_ROOT_PATH . "/" . $dirarray[$rootpath], $type = "images");
            $indeximage_select = new icms_form_elements_Select('', 'downfile', '');
            $indeximage_select->addOptionArray($graph_array);
            $indeximage_select->setExtra("onchange='showImgSelected(\"image\", \"downfile\", \"" . $dirarray[$rootpath] . "\", \"\", \"" . XOOPS_URL . "\")'");
            $indeximage_tray = new icms_form_elements_Tray(_AM_WFD_DOWN_FSHOWSELECTEDIMAGE, '&nbsp;');
            $indeximage_tray->addElement($indeximage_select);
            if (!empty($_REQUEST['downfile']))
            {
                $indeximage_tray->addElement(new icms_form_elements_Label('', "<br /><br /><img src='" . XOOPS_URL . "/" . $dirarray[$rootpath] . "/" . $_REQUEST['downfile'] . "' name='image' id='image' alt='' title='image' />"));
            }
            else
            {
            ...
            $iform->addElement(new icms_form_elements_File(_AM_WFD_DOWN_FUPLOADIMAGE, 'uploadfile', 0));
            ...
            $butt_dup = new icms_form_elements_Button('', '', _AM_WFD_BUPLOAD, 'submit');
            $butt_dup->setExtra('onclick="this.form.elements.op.value=\'upload\'"');
            $dup_tray->addElement($butt_dup);

            $butt_dupct = new icms_form_elements_Button('', '', _AM_WFD_BDELETEIMAGE, 'submit');
