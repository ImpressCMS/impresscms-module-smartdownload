...
global $xoopsUser, $xoopsModuleConfig, $myts;
	...
	if($download->isNew()) {redirect_header(WFDOWNLOADS_URL.'index.php', 1, _MD_WFD_NODOWNLOAD);}
	...
	if($agreed == 0)
	{
		if ($xoopsModuleConfig['check_host'])
		{
			...
			if(!$goodhost) {redirect_header(WFDOWNLOADS_URL.'/modules/wfdownloads/singlefile.php?cid=$cid&amp;lid=$lid', 20, _MD_WFD_NOPERMISETOLINK);}
		}
	}
	...
	else
	{
		...
		if($isadmin == false) {$download_handler->incrementHits($lid);}
		//--
	...
	elseif(!empty($full_name))
	{
		...
		if($temp_name == '') {$file_name = $full_name;}
		else {$file_name = $temp_name;}
...
//		header("Content-Length: ".filesize($filePath));
		...
		if(isset($mimeType) && strstr($mimeType, "text/")) {wfd_download($filePath, false, true);}
		else {wfd_download($filePath, true, true);}
		exit();
	}
	else
	{
