<?php
/**
 * $Id: mirror.php,v 1.7 2007/09/30 12:39:13 m0nty_ Exp $
 * Module: WF-Downloads
 * Version: v3.1
...
switch ($op)
{
    ...
    $mirror_handler = icms_getModuleHandler('mirror');
    $criteria = new icms_db_criteria_Item("submit", 0);
    $criteria2 = new icms_db_criteria_Item("submit", 1);
    ...
    if ($num > 0)
    {
        ...
        $downloads = $download_handler->getList(new icms_db_criteria_Item("lid", "(".implode(',', $lids).")", "IN"));
        $users = $member_handler->getUserList(new icms_db_criteria_Item("uid", "(".implode(',', $uids).")"));
        ...
        foreach (array_keys($mirrors) as $i) {
            ...
            $status = "<a href='".WFDOWNLOADS_URL."admin/mirror.php?op=approve_mirror&mirror_id=" . $mirror_id . "'>" . $imagearray['approve'] . "</a>";
            $modify = "<a href='".WFDOWNLOADS_URL."admin/mirror.php?op=edit_mirror&mirror_id=" . $mirror_id . "'>" . $imagearray['editimg'] . "</a>";
    ...
    if ($num2 > 0)
    {
        ...
        $downloads = $download_handler->getList(new icms_db_criteria_Item("lid", "(".implode(',', $lids2).")", "IN"));
        $users = $member_handler->getUserList(new icms_db_criteria_Item("uid", "(".implode(',', $uids2).")"));
    ...
    $pagenav = new icms_view_PageNav($num, $xoopsModuleConfig['admin_perpage'] , $start, 'start');
