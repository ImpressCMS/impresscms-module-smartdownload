<?php
/**
 * $Id: mimetypes.php,v 1.5 2007/09/30 12:39:13 m0nty_ Exp $
 * Module: WF-Downloads
 * Version: v2.0.5a
...
function edit_mimetype($mime_id = 0)
{
    ...
    $mime_handler = icms_getModuleHandler('mimetype');
    ...
    $sform = new icms_form_Theme($forminfo , "op", $_SERVER['REQUEST_URI']);
    $sform -> addElement(new icms_form_elements_Text(_AM_WFD_MIME_EXTF, 'mime_ext', 5, 60, $mimetype->getVar('mime_ext', 'e')), true);
    $sform -> addElement(new icms_form_elements_Text(_AM_WFD_MIME_NAMEF, 'mime_name', 50, 255, $mimetype->getVar('mime_name', 'e')), true);
    $sform -> addElement(new icms_form_elements_Textarea(_AM_WFD_MIME_TYPEF, 'mime_type', $mimetype->getVar('mime_types', 'e'), 7, 60));
    $madmin_radio = new icms_form_elements_Radioyn(_AM_WFD_MIME_ADMINF, 'mime_admin', $mimetype->getVar('mime_admin', 'e'), ' ' . _YES . '', ' ' . _NO . '');
    $sform -> addElement($madmin_radio);
    $muser_radio = new icms_form_elements_Radioyn(_AM_WFD_MIME_USERF, 'mime_user', $mimetype->getVar('mime_user', 'e'), ' ' . _YES . '', ' ' . _NO . '');
    ...
    if (!$mime_id)
    {
        $butt_create = new icms_form_elements_Button('', '', _AM_WFD_MIME_CREATE, 'submit');
        $butt_create -> setExtra('onclick="this.form.elements.op.value=\'save\'"');
        $button_tray -> addElement($butt_create);

        $butt_clear = new icms_form_elements_Button('', '', _AM_WFD_MIME_CLEAR, 'reset');
        $button_tray -> addElement($butt_clear);

        $butt_cancel = new icms_form_elements_Button('', '', _AM_WFD_MIME_CANCEL, 'button');
        $butt_cancel -> setExtra('onclick="history.go(-1)"');
        $button_tray -> addElement($butt_cancel);
    }
    else
    {
        $sform->addElement(new icms_form_elements_Hidden('mime_id', $mime_id));
        $butt_create = new icms_form_elements_Button('', '', _AM_WFD_MIME_MODIFY, 'submit');
        $butt_create -> setExtra('onclick="this.form.elements.op.value=\'save\'"');
        $button_tray -> addElement($butt_create);
    ...
    $iform = new icms_form_Theme(_AM_WFD_MIME_FINDMIMETYPE, "op", $_SERVER['REQUEST_URI']);
    $iform -> addElement(new icms_form_elements_Text(_AM_WFD_MIME_EXTFIND, 'fileext', 5, 60, ""), true);
    $button_tray = new icms_form_elements_Tray('', '');
    $hidden = new icms_form_elements_Hidden('op', 'openurl');
    $button_tray -> addElement($hidden);
    $butt_create = new icms_form_elements_Button('', '', _AM_WFD_MIME_FINDIT, 'submit');
...
switch ($op)
{
	...
	echo "
        ...
        $pagenav = new icms_view_PageNav($mime_num, 20, $start, 'start');
