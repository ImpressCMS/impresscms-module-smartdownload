...
function wfdownloads_search($queryarray, $andor, $limit, $offset, $userid = 0)
{
    ...
    $criteria = new icms_db_criteria_Compo(new icms_db_criteria_Item("cid", "(".implode(',', $allowed_cats).")", "IN"));
    if ($userid != 0)
    {
        $criteria->add(new icms_db_criteria_Item("submitter", intval($userid)));
    }
...
//        $subcrit = new icms_db_criteria_Compo(new icms_db_criteria_Item("title", "%".$queryarray[0]."%", 'LIKE'), 'OR');
//        $subcrit->add(new icms_db_criteria_Item("description", "%".$queryarray[0]."%", 'LIKE'), 'OR');
//        $criteria->add($subcrit);
//        unset($subcrit);

	  $allsubcrits = new icms_db_criteria_Compo(); // added to fix bug related to nesting of ( )
      for($i = 0;$i < $count;$i++) // 1 changed to 0 so everything happens in one loop now
      {
            $subcrit = new icms_db_criteria_Compo(new icms_db_criteria_Item("title", "%".$queryarray[$i]."%", 'LIKE'), 'OR');
            $subcrit->add(new icms_db_criteria_Item("description", "%".$queryarray[$i]."%", 'LIKE'), 'OR');
...
*/
	...
	$category_handler = icms_getModuleHandler('category', 'wfdownloads');
	...
	{
		$ele_criteria = new icms_db_criteria_Compo();
		$ele_criteria->add(new icms_db_criteria_Item('ele_display', 1), 'OR');
		foreach($groups as $thisgroup)
		{
			$ele_criteria->add(new icms_db_criteria_Item('ele_display', '%,'.$thisgroup.',%', 'LIKE'), 'OR');
		}
	...
	for($i = 0;$i < $count;$i++)
	{
		...
		$allsubcrits = new icms_db_criteria_Compo();
	    $thisSearchTerm = count($queryarray) > 0 ? $queryarray[$i] : "";
	    $subcrit = new icms_db_criteria_Compo(new icms_db_criteria_Item("title", "%".$thisSearchTerm."%", 'LIKE'), 'OR');
	    $subcrit->add(new icms_db_criteria_Item("description", "%".$thisSearchTerm."%", 'LIKE'), 'OR');
		...
		foreach($fids as $fid)
		{
			if(!isset($formulize_mgr)) { $formulize_mgr = icms_getModuleHandler('elements', 'formulize'); } 
		...
		{
			$subs_plus_custom = new icms_db_criteria_Compo(new icms_db_criteria_Item("formulize_idreq", "(".implode(',', $saved_ids).")", "IN"));
