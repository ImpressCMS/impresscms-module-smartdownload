...
class XoopsPersistableObjectHandler extends icms_core_object {
    ...
    function &get($id, $as_object = true) {
        if (is_array($this->keyName)) {
            ...
            for ($i = 0; $i < count($this->keyName); $i++) {
                $criteria->add(new icms_db_criteria_Item($this->keyName[$i], intval($id[$i])));
            }
        }
        else {
            $criteria = new icms_db_criteria_Item($this->keyName, intval($id));
        }
        $criteria->setLimit(1);
